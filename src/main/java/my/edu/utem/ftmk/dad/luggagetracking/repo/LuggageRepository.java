...
public interface LuggageRepository extends JpaRepository<Luggage, Long>{
	...
	@Query("SELECT l FROM Luggage l WHERE l.passengerFlight.flight.flightNo = "
			+ ":flightNo AND l.status.id = 6")
	List<Luggage> findUnclaimedLuggage(@Param("flightNo") String flightNo);

	@Query("SELECT l FROM Luggage l WHERE l.passengerFlight.flight.flightNo = "
			+ ":flightNo AND l.status.id = 7")
	List<Luggage> findMissingLuggage(@Param("flightNo") String flightNo);
	
	@Query(value = "SELECT l.RFID AS rfid, " +
	        ...
	        "GROUP BY l.RFID, s.Name", nativeQuery = true)
	List<Object[]> findLuggageCheckPointTimeByFlightNo(@Param("flightNo") String flightNo);
